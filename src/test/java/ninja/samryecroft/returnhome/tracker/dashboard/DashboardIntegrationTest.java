...
class DashboardIntegrationTest extends AbstractIntegrationTest {
    ...
    void viewerSeesOnlyTheirAssignedHomeNotTheWholeCareProviderOrganisation() throws Exception {
        ...
        // scoped to exactly one young person read "1 children" (now "1 young people"). Assigned home has exactly one child here
        // (saveLiveRequest creates it), so this is the real singular case, not a contrived count.
        assertThat(html).contains("1 young person ·");
        assertThat(html).doesNotContain("1 young people");
    }
