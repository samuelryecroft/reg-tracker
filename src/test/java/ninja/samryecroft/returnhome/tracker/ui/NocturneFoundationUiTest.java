...
class NocturneFoundationUiTest extends AbstractUiTest {
     ...
     * {@code .icon} carries {@code width: 1em; height: 1em} (app.css), so the box is a full 16x16
     * whether the sprite reference resolved or not - measured in headless Chrome against the real
     * vendored sprite, a valid symbol id, a typo'd symbol id and a 404'd sprite path all report
     * exactly 16x16. The {@code <use>} is the element that resolves or doesn't: 15.5x9.0 for a real
     * glyph, 0x0 for both failure modes. Reserved space is not a rendered icon.
     *
